...
void
WuQuantizer::AddPixel(int x, int y, uint8_t r, uint8_t g, uint8_t b) {

	Hist3DAddPixel(wt, mr, mg, mb, gm2, x, y, r, g, b);
}

void
WuQuantizer::Hist3DAddPixel(LONG *vwt, LONG *vmr, LONG *vmg, LONG *vmb, float *m2, int x, int y, uint8_t r, uint8_t g, uint8_t b) {
	int ind = 0;
...
int
WuQuantizer::Build(int PaletteSize, void* ctx, void(*Visitor)(void* ctx, int i, uint8_t r, uint8_t g, uint8_t b)) {
	BYTE *tag = NULL;
	...
	for (k = 0; k < PaletteSize ; k++) {
		...
		if (weight) {
			rgbRed	= (BYTE)(((float)Vol(&cube[k], mr) / (float)weight) + 0.5f);
			rgbGreen = (BYTE)(((float)Vol(&cube[k], mg) / (float)weight) + 0.5f);
			rgbBlue	= (BYTE)(((float)Vol(&cube[k], mb) / (float)weight) + 0.5f);
		} else {
			// Error: bogus box 'k'

			rgbRed = rgbGreen = rgbBlue = 0;
		}
		Visitor(ctx, k, rgbRed, rgbGreen, rgbBlue);
	}
